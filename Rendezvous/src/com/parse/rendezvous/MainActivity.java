...
public class MainActivity extends FragmentActivity implements LocationListener,
  ...
  private ArrayList<List<Object>> attendees;
  private String friendsList;
  Button push;

  // Adapter for the Parse query
  private ParseQueryAdapter<RendezvousPost> postsQueryAdapter;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    Log.d("" + ob.size(), "okay");
    attendees = new ArrayList<List<Object>>();
    //ArrayList<String> obj = ob.get(0).get("Attendees");
    //Log.d(((JSONArray) ob.get(0).get("Attendees")).get(0).toString(), "yuuuu");
    for (ParseObject event : ob) {
    	attendees.add(event.getList("Attendees"));
    	//ArrayList<String> arr = new ArrayList<String>();
    	//arr = (ArrayList<String>) event.get("Attendees");
    	//if (arr != null)
    	//attendees.add(arr);
    }
    //Log.d(attendees.get(0).toString(), "attend");
    String userName = ParseUser.getCurrentUser().getUsername();
    int x = 0;
    for (List<Object> i : attendees){
    ...
    List<Object> friends = attendees.get(x);
    final String [] friendsArr = new String[friends.size()];
    for (int i= 0; i < friends.size(); i++)
    {
    	friendsArr[i] = (String) friends.get(i);
    }
    friendsList = Arrays.toString(friendsArr);
    ListView listView = (ListView) findViewById(R.id.friend_view);
    /*
    listView.setAdapter(new ArrayAdapter<String>(MainActivity.this,
 	R.layout.listview, friendsArr));
    ...
    ParseQueryAdapter.QueryFactory<RendezvousPost> factory =
        new ParseQueryAdapter.QueryFactory<RendezvousPost>() {
          public ParseQuery<RendezvousPost> create() {
            Location myLoc = (currentLocation == null) ? lastLocation : currentLocation;
            ParseQuery<RendezvousPost> query = RendezvousPost.getQuery();
            query.include("user");
    ...
    postsQueryAdapter = new ParseQueryAdapter<RendezvousPost>(this, factory) {
      @Override
      public View getItemView(RendezvousPost post, View view, ViewGroup parent) {
        if (view == null) {
          view = View.inflate(getContext(), R.layout.rendezvous_post_item, null);
        }
        TextView contentView = (TextView) view.findViewById(R.id.content_view);
        TextView usernameView = (TextView) view.findViewById(R.id.username_view);
        contentView.setText(post.getText());
        usernameView.setText(post.getUser().getUsername());
        TextView listView = (TextView) view.findViewById(R.id.friend_view);
        listView.setText(friendsList);
        return view;
      }
    };
    ...
    postsListView.setOnItemClickListener(new OnItemClickListener() {
      public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        final RendezvousPost item = postsQueryAdapter.getItem(position);
  ...
  private void doMapQuery() {
    ...
    ParseQuery<RendezvousPost> mapQuery = RendezvousPost.getQuery();
    ...
    mapQuery.findInBackground(new FindCallback<RendezvousPost>() {
      @Override
      public void done(List<RendezvousPost> objects, ParseException e) {
        if (e != null) {
          if (Application.APPDEBUG) {
        ...
        for (RendezvousPost post : objects) {
